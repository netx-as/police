...
sub ScanClient {
	...
	push(@request, sprintf "\t\t<services/>\n") if ($self->{Config}->GetValBool("servicediff"));
...
sub ErrReport {
	...
	my ($silenterr) = $self->{Config}->GetVal("silenterr");

	# silent errors, escape is set
	if (defined($silenterr) && $silenterr) {
		return ; 
	}
...
sub MkBkpDiffReport {
	...
	foreach my $file  (sort keys  %flist) {
		...
		if ( -l "$old/$file" ) {
			...
			next if (defined($diff{'Flags'}->{'F'}));
			my $ret = system("diff -C 2 \"$old/$file\" \"$new/$file\" > $tempf") if (!defined($diff{'Flags'}->{'F'}));
			if ($ret != 0) {
...
#		next if ( exists $diff->{'Server'} && exists $diff->{'Server'}->{'nonexists'} && !exists $diff->{'Client'} );
		...
		if (defined($diff->{Client})) {
			my $pkgname = undef;
			if ( defined($diff->{Server}) && defined($diff->{Server}->{'package'}) ) { 
				$pkgname = $diff->{Server}->{'package'};
			}
			printf $flist "%-10s %-60s  # %s\n", 
				defined($pkgname) ? $pkgname : "*", 
				$file, 
				DescribeFile(%{$diff->{Client}});
			if ( defined($diff->{Server}) && defined($diff->{Server}->{'package'}) ) { 
				printf "pkg: ".$diff->{Server}->{'package'}."\n";
		#		printf Dumper(\$diff->{Server});
				$real_paths{$file} = $diff->{Server}->{'full_path'};
			}
		}
	}
	...
	open EDLIST, "< $edlist";
	while (my $line = <EDLIST>) { 
		chomp $line;
		my ($pkgname, $filename) = split(/\s+/, $line);
		if (defined($pkgname) && $pkgname ne '*' && $pkgname =~ /^dir:.*/) {
			$edlisth{$filename} = $pkgname; 
		}

		if (defined($pkgname) || $pkgname eq '*') {
			push(@downlist, $filename);
		}
	}
	close EDLIST;

	open EDLIST, "> $edlist";
	#foreach (keys %edlisth) {
	foreach (@downlist) {
		printf EDLIST "%s\n", $_;
	}
	close EDLIST;
	...
	close $hout;
	unlink($edlist);
	$self->{Log}->Progress("done\n");
	$self->{Log}->ProgressStep("downloaded data has been stored into download.tgz\n");
	$self->{Log}->ProgressStep("Unpacking download.tgz to download.police\n");

	system("mkdir download.police");
	system("cd download.police && tar xvzf ../download.tgz ");
	
	foreach (keys %edlisth) {
		my $cmd = sprintf "cp download.police/%s %s", $_, $real_paths{$_};

		printf "CMD %s (y/N)?  \b", $cmd;
		if (<STDIN> =~ /[yY]/ ) {
			system($cmd);
			printf("Done.");
		}
